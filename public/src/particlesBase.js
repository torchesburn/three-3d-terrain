...
class ParticleSystem {
  ...
  _UpdateParticles(timeElapsed) {
    ...
    for (let p of this._particles) {
      ...
      if (log.length < 100) {
        let x = this._alphaSpline.Get(t);
        log.push(x);
        console.log('_alphaSpline:', x, '| t:', (p.life / p.maxLife));
      }
      // console.log('[particle] ', t);
      // if (t > 0.3) {
      //   p.colour.copy(0.1);
      // }
    }
...
class ParticleSystemDemo {
  //------------------------------------------------------------
  constructor(params = {}) {
    this.targetElementId = params.targetElementId || 'target'
    this._Initialize();
  }
  ...
  _Initialize() {
    ...
    this._camera.position.set(55, 10, 0);
